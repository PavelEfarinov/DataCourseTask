...
public class JwtService {
    ...
    public Optional<BaseUserEntity> find(String token) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(SECRET);
            JWTVerifier verifier = JWT.require(algorithm)
                    .build();
            DecodedJWT jwt = verifier.verify(token);
            return userRepository.findByLogin(jwt.getClaims().get("userId").asString());
        } catch (JWTVerificationException ignored){
            return Optional.empty();
        }
    }

    private String create(BaseUserEntity user) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(SECRET);
            return JWT.create().withClaim("userId", user.getLogin()).sign(algorithm);
